...
def handle_session_logic(selected_session_id_from_ui):
    ...
    if selected_session_id_from_ui == "➕ Perbualan Baru":
        if st.session_state.session_id != "new": # Jika bertukar DARI sesi sedia ada KE baru
            ...
            st.session_state.current_filename_prefix = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.session_state.chat_page_num = 1
            # st.rerun() # Rerun akan berlaku secara semula jadi jika widget berubah, atau jika perlu, tambah di sini
...
# --- KOMPONEN UI (Dengan Cadangan Perubahan) ---

def display_sidebar(available_models_list):
    # st.sidebar.title("⚙️ Konfigurasi") # Tajuk utama sidebar boleh diletakkan di atas sekali
    # st.sidebar.markdown("---") # Garisan pembahagi awal

    with st.sidebar: # Gunakan 'with st.sidebar:' untuk kumpulan yang lebih jelas
        st.markdown("## ⚙️ Tetapan & Sesi") # Tajuk utama yang lebih ringkas
        st.markdown("---")
        ...
        if available_models_list:
            ...
            selected_model_ui = st.selectbox(
                ...
                label_visibility="collapsed" # Sembunyikan label jika tajuk di atas sudah cukup
            )
        ...
        # Maklumat Tambahan (jika perlu, boleh juga dalam expander)
        # with st.expander("ℹ️ Info Tambahan", expanded=False):
        #     st.caption(f"URL Ollama: `{OLLAMA_BASE_URL}`")
        #     st.caption(f"Logo: `{LOGO_PATH if os.path.exists(LOGO_PATH) else 'Tidak ditemui'}`")
        #     st.caption(f"Tera Air: `{WATERMARK_TEXT}`")

    return selected_session_id_ui

def display_chat_messages_paginated():
    # st.subheader("📜 Perbualan") # Boleh dikekalkan atau diintegrasikan dengan lebih baik
    ...
    # Paginasi di bahagian bawah atau atas, dengan gaya yang lebih minimalis
    # Kita boleh letakkan paginasi dalam satu baris dengan maklumat lain jika perlu
    
    # Papar mesej
    # Tidak banyak yang boleh diubah pada st.chat_message itu sendiri, tetapi kita pastikan ruang sekelilingnya bersih.
    ...
    for msg in messages_to_display:
        with st.chat_message(msg["role"]):
            ...
            if msg["role"] == "assistant" and "time_taken" in msg and msg["time_taken"] is not None:
                st.caption(f"⏱️ {msg['time_taken']:.2f}s") # Ikon untuk masa

    # Paginasi di bahagian bawah
    if max_page > 1:
        cols_pagination = st.columns([1, 3, 1]) # [Kosong, Slider, Kosong] untuk tengahkan
...
def display_export_options():
    if not st.session_state.chat_history:
        # st.info("Tiada perbualan untuk dieksport.") # Tidak perlu jika tiada apa-apa untuk dieksport
        return

    st.markdown("---") # Pembahagi sebelum bahagian eksport
    with st.expander("📤 Eksport Perbualan", expanded=False): # Letakkan dalam expander
        ...
        # Tidak perlu kemas kini st.session_state.current_filename_prefix di sini,
        # ia akan diambil nilainya semasa butang "Eksport" ditekan.
        ...
        if st.button("📁 Eksport Sekarang", key="export_main_button", type="primary", use_container_width=True):
            ...
            filename_base = custom_filename_prefix_ui # Guna nilai terkini dari input
...
def main():
        ...
        if uploaded_file is not None:
            ...
            if extracted_text:
                ...
                st.session_state.chat_history.append({
                    "role": "assistant", "content": assistant_response, "time_taken": gen_time
                })
    ...
    if user_input:
        ...
        st.session_state.chat_history.append({
            "role": "assistant", "content": assistant_response_text, "time_taken": generation_time
        })
        if st.session_state.session_id == "new":
            st.session_state.session_id = st.session_state.current_filename_prefix
        save_chat_session(st.session_state.session_id, st.session_state.chat_history)
